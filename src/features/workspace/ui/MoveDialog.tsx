  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4">
      <form
        ...
        className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/90 p-6 shadow-xl text-slate-100"
      >
        <h2 className="text-lg font-semibold text-slate-100">Move {itemDescriptor}</h2>
        <p className="mt-1 text-sm text-slate-400">
          <span className="text-slate-200">{itemLabel}</span>
        </p>
        <p className="mt-3 text-sm text-slate-300">
          Current location: <span className="font-medium text-emerald-300">{currentLocationLabel}</span>
        </p>
        {itemType === "folder" && itemPathLabel ? (
          <p className="mt-1 text-xs text-slate-500">Folder path: {itemPathLabel}</p>
        ) : null}
        ...
        <label className="mt-5 block text-sm text-slate-200">
          ...
          <select
            ...
            disabled={busy}
            className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
          >
          ...
          <div className="mt-3">
            <label className="block text-xs text-slate-300">
              ...
              <input
                ...
                disabled={busy}
                className="mt-2 w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                placeholder={newFolderDefaultPath}
              />
            </label>
            <p className="mt-1 text-xs text-slate-500">Enter the destination folder name. It will be created if needed.</p>
          </div>
        ) : null}
        <p className="mt-2 text-xs text-slate-500">{destinationHint}</p>
        {selectionDisabled ? (
          <p className="mt-1 text-xs text-amber-400">You cannot move a folder into itself or one of its subfolders.</p>
        ) : null}
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            ...
            onClick={onCancel}
            disabled={busy}
            className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-200 transition hover:border-slate-600 disabled:cursor-not-allowed disabled:opacity-60"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={!canSubmit}
            className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {moveVerb}
          </button>
        </div>
