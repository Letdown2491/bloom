...
vi.mock("./components/StatusFooter", () => ({
  StatusFooter: () => null,
}));
...
vi.mock("./components/MainNavigation", () => {
  ...
  const MainNavigation = ({ tab, onSelectTab }: Props) => (
    <nav>
      <span data-testid="active-tab">{tab}</span>
      <button type="button" onClick={() => onSelectTab("upload")}>
        Open Upload Tab
      </button>
      <button type="button" onClick={() => onSelectTab("browse")}>
        Open Browse Tab
      </button>
    </nav>
  );
...
vi.mock("./components/WorkspaceSection", () => {
  type Props = {
    tab: string;
...
describe("App upload return flow", () => {
  it("restores the previous browse context after a successful upload", async () => {
    ...
    const { __mockState, __mockActiveList } = await import("./components/WorkspaceSection");
