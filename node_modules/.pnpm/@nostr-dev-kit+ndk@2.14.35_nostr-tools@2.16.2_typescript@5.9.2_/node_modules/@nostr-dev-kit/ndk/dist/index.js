...
var SignatureVerificationStats = class {
  ...
  constructor(ndk, intervalMs = 1e4) {
    this.ndk = ndk;
    this.debug = (0, import_debug.default)("ndk:signature-verification-stats");
...
// src/relay/sets/calculate.ts
var d = (0, import_debug3.default)("ndk:outbox:calculate");
...
var NDKPrivateKeySigner = class _NDKPrivateKeySigner {
  ...
  constructor(privateKeyOrNsec, ndk) {
    if (typeof privateKeyOrNsec === "string") {
      if (privateKeyOrNsec.startsWith("nsec1")) {
        const { type, data } = import_nostr_tools5.nip19.decode(privateKeyOrNsec);
        if (type === "nsec") this._privateKey = data;
        else throw new Error("Invalid private key provided.");
      } else if (privateKeyOrNsec.length === 64) {
        this._privateKey = (0, import_utils2.hexToBytes)(privateKeyOrNsec);
      } else {
    ...
    this._pubkey = (0, import_nostr_tools4.getPublicKey)(this._privateKey);
  ...
  get privateKey() {
    ...
    return (0, import_utils2.bytesToHex)(this._privateKey);
  }
  /**
  ...
  get nsec() {
    if (!this._privateKey) throw new Error("Not ready");
    return import_nostr_tools5.nip19.nsecEncode(this._privateKey);
  }
  /**
  ...
  get npub() {
    if (!this._pubkey) throw new Error("Not ready");
    return import_nostr_tools5.nip19.npubEncode(this._pubkey);
  }
  /**
   * Generate a new private key.
   */
  static generate() {
    const privateKey = (0, import_nostr_tools4.generateSecretKey)();
  ...
  async sign(event) {
    ...
    return (0, import_nostr_tools4.finalizeEvent)(event, this._privateKey).sig;
  }
  ...
  async encrypt(recipient, value, scheme) {
    ...
    if (scheme === "nip44") {
      const conversationKey = import_nostr_tools4.nip44.v2.utils.getConversationKey(this._privateKey, recipientHexPubKey);
      return await import_nostr_tools4.nip44.v2.encrypt(value, conversationKey);
    }
    return await import_nostr_tools4.nip04.encrypt(this._privateKey, recipientHexPubKey, value);
  }
  async decrypt(sender, value, scheme) {
    ...
    if (scheme === "nip44") {
      const conversationKey = import_nostr_tools4.nip44.v2.utils.getConversationKey(this._privateKey, senderHexPubKey);
      return await import_nostr_tools4.nip44.v2.decrypt(value, conversationKey);
    }
    return await import_nostr_tools4.nip04.decrypt(this._privateKey, senderHexPubKey, value);
  }
  /**
...
function filterForEventsTaggingId(id) {
  try {
    const decoded = import_nostr_tools6.nip19.decode(id);
    switch (decoded.type) {
      case "naddr":
        return {
...
function filterFromId(id) {
  ...
  if (id.match(BECH32_REGEX)) {
    try {
      decoded = import_nostr_tools6.nip19.decode(id);
      switch (decoded.type) {
        case "nevent": {
...
function relaysFromBech32(bech322, ndk) {
  try {
    const decoded = import_nostr_tools6.nip19.decode(bech322);
...
var NDKUser = class _NDKUser {
  ...
  constructor(opts) {
    ...
    if (opts.nprofile) {
      try {
        const decoded = import_nostr_tools7.nip19.decode(opts.nprofile);
...
function getRumorEvent(event, kind) {
  ...
  rumor.sig = void 0;
  rumor.id = (0, import_nostr_tools8.getEventHash)(rumor);
  return new NDKEvent(event.ndk, rumor);
}
...
var NDKSubscriptionManager = class {
  ...
  dispatchEvent(event, relay, optimisticPublish = false) {
    ...
    for (const sub of subscriptions) {
      if ((0, import_nostr_tools9.matchFilters)(sub.filters, event)) {
...
function getEntity(entity) {
  try {
    const decoded = import_nostr_tools10.nip19.decode(entity);
...
async function generateZapRequest(target, ndk, data, pubkey, amount, relays, comment, tags, signer) {
  ...
  const zapRequest = import_nostr_tools11.nip57.makeZapRequest({
    profile: pubkey,
